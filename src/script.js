...
import { exoplanets } from './exoplanets/exoplanetData.js';
...
// Default view target: a point slightly in front of the camera so the controls don't always pivot around the Sun
const defaultViewTarget = camera.position.clone().add(new THREE.Vector3(0, 0, -1).applyQuaternion(camera.quaternion).multiplyScalar(10));
controls.target.copy(defaultViewTarget);
controls.update();
...
function identifyPlanet(clickedObject) {
  // Check exoplanets first
  const exoplanet = exoplanetObjects.find(exo => clickedObject.material === exo.planet.material);
  if (exoplanet) {
    offset = 20;
    return { ...exoplanet, isExoplanet: true };
  }
...
// Function to display exoplanet information
function showExoplanetInfo(exoplanet) {
    const info = document.getElementById('exoplanetInfo');
    const name = document.getElementById('exoplanetName');
    const dataGrid = document.getElementById('exoplanetData');
    const analysisResult = document.getElementById('analysisResult');
    
    name.innerText = exoplanet.name;
    ...
    // Add all KOI data
  // Render each of the required fields in a clear grid
  const fields = [
    'koi_period','koi_time0bk','koi_impact','koi_duration','koi_depth','koi_prad','koi_teq',
    'koi_insol','koi_model_snr','koi_tce_plnt_num','koi_tce_delivname','koi_steff','koi_slogg',
    'koi_srad','ra','dec','koi_kepmag','koi_fittype','koi_score'
  ];

  fields.forEach(key => {
    const value = exoplanet[key];
    const item = document.createElement('div');
    item.className = 'data-item';
    item.innerHTML = `<span class="data-label">${key}:</span> ${value !== undefined ? value : 'N/A'}`;
...
// Function to close exoplanet info panel
function closeExoplanetInfo() {
    const info = document.getElementById('exoplanetInfo');
...
window.closeExoplanetInfo = closeExoplanetInfo;

// Function to analyze exoplanet type
async function analyzeExoplanet() {
    const analysisResult = document.getElementById('analysisResult');
    analysisResult.style.display = 'block';
    
    if (selectedPlanet && selectedPlanet.isExoplanet) {
        const data = selectedPlanet.data;
        
        // Simulated ML model analysis (replace with actual API call to your model)
    // Assemble only the required payload fields
    const payload = {
      ...
      koi_tce_plnt_num: data.koi_tce_plnt_num,
      koi_tce_delivname: data.koi_tce_delivname,
      koi_steff: data.koi_steff,
      koi_slogg: data.koi_slogg,
      koi_srad: data.koi_srad,
      ra: data.ra,
      dec: data.dec,
      koi_kepmag: data.koi_kepmag,
      koi_fittype: data.koi_fittype,
      koi_score: data.koi_score
    };

    analysisResult.innerHTML = '<p>Sending data to model (simulated)...</p>';

    // Simulate API delay
    await new Promise(resolve => setTimeout(resolve, 1200));

    // Hard-coded response for testing (replace with real API call)
    const simulatedResponse = {
      predicted_type: data.koi_prad < 1.6 ? 'Rocky' : (data.koi_prad < 3.9 ? 'Super-Earth' : 'Gas Giant'),
      confidence: Math.min(0.99, Math.max(0.5, data.koi_score || 0.8))
    };

    analysisResult.innerHTML = `
      <p><strong>Analysis Complete (simulated)</strong></p>
      <p>Sent payload: <small>${JSON.stringify(payload)}</small></p>
      <p>Predicted Type: <strong>${simulatedResponse.predicted_type}</strong></p>
      <p>Confidence: <strong>${(simulatedResponse.confidence*100).toFixed(1)}%</strong></p>
    `;
    }
}
window.analyzeExoplanet = analyzeExoplanet;

// ******  SHOW PLANET INFO AFTER SELECTION  ******
function showPlanetInfo(planet) {
  // If a string was passed (legacy), show the solar-system planet info
  ...
  // If an object representing an exoplanet was passed
  if (planet && planet.isExoplanet) {
    showExoplanetInfo(planet.data);
    return;
  }

  // If an object representing a solar-system planet (returned from createPlanet) was passed
  if (planet && planet.name && planetData[planet.name]) {
    const info = document.getElementById('planetInfo');
    const name = document.getElementById('planetName');
    const details = document.getElementById('planetDetails');

    name.innerText = planet.name;
    details.innerText = `Radius: ${planetData[planet.name].radius}\nTilt: ${planetData[planet.name].tilt}\nRotation: ${planetData[planet.name].rotation}\nOrbit: ${planetData[planet.name].orbit}\nDistance: ${planetData[planet.name].distance}\nMoons: ${planetData[planet.name].moons}\nInfo: ${planetData[planet.name].info}`;
...
const exoplanetSystemGroup = new THREE.Group();
scene.add(exoplanetSystemGroup);

exoplanets.forEach((exoData, index) => {
    const exoplanetGeom = new THREE.SphereGeometry(exoData.size, 32, 20);
    const exoplanetMat = new THREE.MeshPhongMaterial({
        map: loadTexture.load(exoData.texture),
        emissive: 0x112244,
        emissiveIntensity: 0.1
    });
    const exoplanet = new THREE.Mesh(exoplanetGeom, exoplanetMat);
    
    // Position exoplanets in an outer ring
    const angle = (index / exoplanets.length) * Math.PI * 2;
    const distance = 400; // Distance from sun
    exoplanet.position.x = Math.cos(angle) * distance;
    exoplanet.position.z = Math.sin(angle) * distance;
    
    const exoplanetSystem = new THREE.Group();
    exoplanetSystem.add(exoplanet);
    exoplanetSystemGroup.add(exoplanetSystem);
    
    exoplanetObjects.push({
        name: exoData.name,
        planet: exoplanet,
        planetSystem: exoplanetSystem,
        data: exoData
    });
    ...
    const pathPoints = orbitPath.getPoints(100);
    const orbitGeometry = new THREE.BufferGeometry().setFromPoints(pathPoints);
    const orbitMaterial = new THREE.LineBasicMaterial({ color: 0x4a9eff, transparent: true, opacity: 0.1 });
    const orbit = new THREE.LineLoop(orbitGeometry, orbitMaterial);
    orbit.rotation.x = Math.PI / 2;
    exoplanetSystem.add(orbit);
});
...
const raycastTargets = [
    ...
    ...exoplanetObjects.map(exo => exo.planet)
];
...
function animate(){
  ...
  // Animate exoplanets
  exoplanetObjects.forEach((exo, index) => {
    exo.planet.rotateY(0.002 * settings.acceleration);
    exo.planetSystem.rotateY(0.0001 * settings.accelerationOrbit * (1 / (index + 2)));
  });
